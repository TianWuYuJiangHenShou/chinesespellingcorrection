...
def evaluate(FLAGS, sess, model, data_processor, label_list=None):
    ...
    for step in range(test_num // batch_size):
        if model.multi_task is True: 
            ...
            #print('inputs:',inputs.shape,'preds:',preds.shape,'py_pred:',py_pred.shape,'gmask',gmask.shape,
    ...
    #print('\n')
    #print('all_preds:',all_preds[:5])
    #print('\n')
    #print('all_fusino_preds:',all_fusino_preds[:5])
    #print('\n')
...
def train(FLAGS):
    ...
    print(len(data_processor.get_label_list()))
    np.save(os.path.join(out_dir,'{}'.format('PYID2SEQ'.lower())),data_processor.PYID2SEQ)
    np.save(os.path.join(out_dir,'{}'.format('zi_py_matrix'.lower())),data_processor.get_zi_py_matrix())
    np.save(os.path.join(out_dir,'{}'.format('SKID2SEQ'.lower())),data_processor.SKID2SEQ)
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        for epoch in tqdm(range(EPOCH)):
            for step in tqdm(range(int(train_num / batch_size))):
                total_step += 1
